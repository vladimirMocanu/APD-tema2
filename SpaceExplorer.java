...
public class SpaceExplorer extends Thread {
	...
	public void run() {
		...
		while (true) {
			...
			Message message1 = channel.getMessageHeadQuarterChannel();

			if (message1.getData() == "EXIT") {
				return;

			} else if (message1.getData() != "END") {

				Message message2 = channel.getMessageHeadQuarterChannel();

				int parentSystem = message1.getCurrentSolarSystem();
				int currentSystem = message2.getCurrentSolarSystem();

				if (!discovered.contains(currentSystem)) {

					Message message3 = new Message(parentSystem, currentSystem,
					 encryptMultipleTimes(message2.getData(), hashCount));

					discovered.add(currentSystem);
					channel.putMessageSpaceExplorerChannel(message3);
				}
			}
		}
	}
